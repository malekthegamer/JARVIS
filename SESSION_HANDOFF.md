...
6. **Candidates now that the real-FS surface is complete (slices 32-33):** **PowerShell as a second `run_shell` backend** (cmd.exe only); quality/robustness (memory retrieval round 3 — drive down the ~18% paraphrase miss; vision false-refusal reduction below ~2.3%); new surfaces (multi-brain OpenAI/Claude/Ollama; inbox reading/triage; a `make_folder` verb — deferred from slice 33). (Undo shipped slice 26; audit viewer 28; scroll+double/right-click 29; caged file authoring 30; clipboard 31; real-FS access 32-33 — `drag`/`move_mouse`/`wifi`/horizontal-scroll remain documented *deliberate* deferrals.)
