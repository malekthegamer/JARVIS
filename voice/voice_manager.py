...
Callers (main.py, server.py) never know which concrete provider is behind the
interface. Registered as a settings reload listener, so a dashboard Save &
Apply hot-swaps providers with no restart.

TTS "auto" resolution: preferred provider (ElevenLabs) if its key is
configured, else edge-tts, else pyttsx3 — so first run always has a voice and
entering an ElevenLabs key upgrades the voice instantly.
"""
...
class VoiceManager:
    ...
    def resolve_tts_name(self) -> str:
        ...
        if active != "auto":
            return active
        preferred = settings.get("tts.preferred", "elevenlabs")
    ...
    def _log_swap(self) -> None:
        audit_log.log_action("voice", "providers_reloaded",
                             {"stt": self.resolve_stt_name(), "tts": self.resolve_tts_name()})
    ...
    def synthesize(self, text: str) -> tuple[bytes | None, str]:
        """Audio bytes for the dashboard + the provider name used."""
        for name in self._fallback_chain():
            provider = registry.get("tts", name)
            if provider is None:
                continue
