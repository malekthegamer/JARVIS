...
# Explicit request timeout (ms): a network stall must surface as a
# ProviderError("connection") rather than hang the THINKING state forever.
    ...
    def _client(self):
        ...
        if self._cached_client is None or self._cached_key != key:
            self._cached_client = genai.Client(
                api_key=key, http_options={"timeout": REQUEST_TIMEOUT_MS})
